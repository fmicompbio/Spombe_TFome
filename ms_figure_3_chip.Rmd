...
We can also create the same heatmap, but without showing the TF promoters (rows)
that are not detected to be bound by any TF.
...
# ... create main heatmap with annotations
hm6 <- Heatmap(mprom[o, oc], name = "hm6",
               col = circlize::colorRamp2(
                   breaks = seq(0, 2, length.out = 64),
                   colors = colorRampPalette(c(binary_heatmap_colors["FALSE"],
                                               main_colors[1],
               ...
               cluster_rows = FALSE, show_row_dend = FALSE,
               cluster_columns = FALSE, show_column_dend = FALSE,
               row_split = droplevels(promtype[o]), row_title_gp = gpar(fontsize = fl),
               column_title = "TF ChIP-seq experiments",
               column_title_gp = gpar(fontsize = fl),
               show_row_names = FALSE, show_column_names = FALSE,
               right_annotation = rowLabels[o],
...
hm_legend <- Legend(labels = c("TFs binding to another TF promoter",
                    ...
                    title = " ", legend_gp = gpar(fill = c("#648FFF", "darkblue")))
...
We construct a network in which the nodes represent TFs (shown in orange if
they were ChIP'ed in this study), and the edges represent detected binding
events. An edge starting at a TF `A` and ending at a TF `B` means that `A`
has been detected to bind the promoter of `B`.

We exclude `untagged`, and also the TFs `zas1` and `SPCC320.03` from the network.
`zas1` has `r sum(mprom["zas1", ] > 0)` bound TFs in its promoter, and 
`SPCC320.03` has `r sum(mprom["SPCC320.03", ] > 0)`, and if these would be
included, the network is hard to visualize.
...
gr <- graph_from_adjacency_matrix(adjm, mode = "directed")
...
V(gr)$vertexclass <- ifelse(adjm[cbind(names(V(gr)), names(V(gr)))] > 0,
                            "Auto-regulated TF",
                            ifelse(names(V(gr)) %in% colnames(mprom),
                                   "ChIP'ed TF", "Other TF"))
V(gr)$shape <- "circle"
eL <- as_edgelist(gr)
...
ggr <- ggraph(as_tbl_graph(gr), 
    ...
    scale_shape_manual(values = c("Other TF" = 21, "ChIP'ed TF" = 21, "Auto-regulated TF" = 22), name = "") +
    scale_fill_manual(values = c("Other TF" = gplots::col2hex(bg_color),
                                 "ChIP'ed TF" = gplots::col2hex(main_colors[5]),
                                 "Auto-regulated TF" = main_colors[5]),
                      name = "") + 
...
gr2 <- graph_from_adjacency_matrix(adjm2, mode = "directed")

## Set attributes for vertices and edges
V(gr2)$vertexclass <- ifelse(adjm[cbind(names(V(gr2)), names(V(gr2)))] > 0,
                             "Auto-regulated TF",
                             ifelse(names(V(gr2)) %in% colnames(mprom),
                                    "ChIP'ed TF", "Other TF"))
V(gr2)$shape <- "circle"
eL <- as_edgelist(gr2)
recipr <- paste0(eL[,1], "-", eL[,2]) %in% paste0(eL[,2], "-", eL[,1]) & eL[,1] != eL[,2]
E(gr2)$weight <- ifelse(recipr, 2, 1)
E(gr2)$edgeclass <- ifelse(recipr, "Reciprocal", "Other")
E(gr2)$color <- ifelse(recipr, gplots::col2hex(main_colors[3]), gplots::col2hex(na_color))
...
# visualize
ggplot(pd, aes(prom.number.overlapping.highconf.peaks.categories, avg_tpm + 0.01)) +
    geom_violin(draw_quantiles = 0.5) +
    scale_y_log10() +
    labs(x = "Number of promoter TF peaks",
         y = "RNA level (TPM + 0.01, log-scale)") +
    theme_cowplot()
```
...
cowplot::plot_grid(
    ...
    axis = "b",
    rel_widths = c(1, 1),
    rel_heights = c(w_peaksHm + 10, w_peaksHm + 20)
)
```
