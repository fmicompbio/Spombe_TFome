...
`r sum(mprom["zas1", ] > 0)` bound TFs in its promoter, and `SPCC320.03` has
`r sum(mprom["SPCC320.03", ] > 0)`, and if these were included, the network
would be hard to clearly visualize.
...
connections to and from `Atf1` (but now also including `zas1` and
`SPCC320.03`):
...
if (file.exists(params$coordsatf1)) {
    ...
    ggr2 <- ggraph(as_tbl_graph(gr2), 
                   layout = as.matrix(coordsAtf1[, c("x", "y")]))
} else {
