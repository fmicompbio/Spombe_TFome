...
for (i in seq_along(sel_motifs)) {
    ...
    gg_ov_counts <- ggplot(pd_counts, aes(percentage, motif, fill = type)) +
        ...
        geom_text(data = pd_counts |> filter(grepl("foreground", type)),
                  ...
                  size = fs / ggplot2::.pt,
                  fontface = "bold") +
        labs(x = "Fraction of motif sites",
             y = element_blank(),
             fill = "Genomic region: ") +
        theme_cowplot(fs) +
        theme(legend.position = "none") +
